...
public struct ShellExecutor {
    ...
    public func run(_ command: String) throws {
        ...
        for signal in signalsToTrap {
            ...
            // By setting it to SIG_IGN, we tell the system to ignore the default behavior for this signal,
            // allowing our custom DispatchSource handler to take precedence without the app terminating immediately.
    ...
    do {
      ...
      if let output = String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines), !output.isEmpty {
        ...
        if kill(pid, SIGKILL) == -1 {
          ...
          throw ProcessError.commandFailed(description: "Failed to kill process \(pid). Reason: \(errorDescription)")
        }
