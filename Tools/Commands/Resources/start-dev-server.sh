...
if "$CODE" $CODE_ARGS --list-extensions | grep "firebase-dataconnect-vscode"; then
  echo "FDC Extension Already Installed"
else
...
fi #Check if FDC extension already exists
...
if test -f "$INSTALL_DIR/firebase"; then
...
fi #Check if Firebase CLI is already installed
...
# Optionally install Flutter + FlutterFire.
if [ "$flutter"x != ""x ]; then
