...
final class CacheTests: XCTestCase {
  ...
  let resultTreeJson = """
    ...
    """
  let cacheProvider = EphemeralCacheProvider(
  ...
  func testBDOReuse() throws {
    do {
      ...
      let rootSdo = try resultsProcessor.normalize(resultTreeJson, cacheProvider: cacheProvider)
      
      let reusedCacheId = "27E85023-D465-4240-82D6-0055AA122406"
      
      let user1 = cacheProvider.dataObject(entityGuid: reusedCacheId)
      let user2 = cacheProvider.dataObject(entityGuid: reusedCacheId)
  ...
  func testDehydrationHydration() throws {
    do {
      ...
      let (dehydratedTree, sdo1) = try resultsProcessor.dehydrateResults(
        resultTreeOneItemJson,
        cacheProvider: cacheProvider
      )
      
      let (hydratedTree, sdo2 ) = try resultsProcessor.hydrateResults(dehydratedTree, cacheProvider: cacheProvider)
      
      XCTAssertEqual(sdo1, sdo2)
