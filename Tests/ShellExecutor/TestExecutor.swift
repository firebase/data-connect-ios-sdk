...
final class ShellExecutorTests: XCTestCase {
    ...
    func testSuccessfulCommand() {
        ...
        XCTAssertNoThrow(try executor.run("true"), "A successful command should not throw an error.")
    }
    ...
    func testFailingCommand() {
        ...
        XCTAssertNoThrow(try executor.run("false"), "A failing command should be handled gracefully, not throw.")
    }
